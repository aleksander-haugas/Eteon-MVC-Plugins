<?php
	...
	*/
    ...
    $sql = "SELECT comment.is_approved, comment.id, comment.page_id, comment.author_name, comment.author_email, comment.ip, comment.author_link, comment.body, comment.created_on, page.title FROM " .
...
<header class="w3-container" style="padding-top:22px">
	<h5><strong><i class="fa fa-comments"></i> <?php echo __('Comments Plugin'); ?> | <?php echo __('Comments'); ?></strong></h5>
</header>
<!-- Termina Header -->
<div class="w3-col l8 s12">
	<div class="w3-margin">
		<div class="w3-justify">
			<?php if ($comments_count > 0){ ?>
				<?php while ($comment = $stmt->fetchObject()){ ?>
					<div class="<?php echo odd_even(); ?> moderate w3-card w3-theme w3-margin-top">
						<div class="w3-bar user">
						...
						<div id="user_comment_details_<?php echo $comment->id; ?>" class="w3-hide">
							...
							<div class="w3-container w3-padding w3-theme-l1 email">
				...
				<?php } } else { ?>
				<div class="w3-row w3-section w3-pale-blue w3-padding">
					<h3><i class="fa fa-info-circle"></i> <?php echo __('No comments found.'); ?></h3>
				</div>
			<?php } ?>
		</div>
        ...
        <div class="w3-border w3-theme-light w3-margin-top">
            <?php
                // First page button
                if ($CurPage != 0) { echo "<a href=" . get_url('plugin/comment/index/') . "0 class=\"w3-left w3-button w3-border-right\">&#10094;&#10094;</a>\n ";
                } else {
                ...
                if ($CurPage == 0) { $prev = '<span class="w3-button w3-left disabled">&#10094; Previous Page</span>';
                    ...
                    $prevpage = $CurPage - 1;
                    $prev = '<a href="' . get_url('plugin/comment/index/') . '' . $prevpage . '" class="w3-button w3-left">&#10094; Previous Page</a>';
                }
                // Next page button
                if ($CurPage == $lastpage) { $next = '<span class="w3-button w3-right disabled">Next Page &#10095;</span>';
                } else {
                    $nextpage = $CurPage + 1;
                    $next = '<a href="' . get_url('plugin/comment/index/') . '' . $nextpage . '" class="w3-button w3-right">Next Page &#10095;</a>';

                }
                // Last page button
                if ($CurPage != $lastpage) { echo "\n<a href=" . get_url('plugin/comment/index/') . "$lastpage class=\"w3-button w3-right w3-border-left\">&#10095;&#10095;</a>";
                } else {
                ...
                for ($i = 0; $i <= $lastpage; $i++) {
                    ...
                    if ($i == $CurPage) {
                        ...
                        echo " <a href=" . get_url('plugin/comment/index/') . "$i class=\"w3-button\">$j</a>\n";
                    }
                }
                echo '</div>';
            ?>
